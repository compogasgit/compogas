<?php class ControllerExtensionFeedTurboProductsRSS extends Controller {
	private $from_charset = 'utf-8';
	public function index() {
		if ($this->config->get('turbo_products_rss_status')) {

			$this->load->model('catalog/product');
			$this->load->model('catalog/category');
			$this->load->model('localisation/currency');
			$this->load->model('tool/image');
			$this->load->language('extension/feed/turbo_products_rss');

			$total =  $this->model_catalog_product->getTotalProducts();
			$limit = $this->config->get('turbo_products_rss_limit') ? $this->config->get('turbo_products_rss_limit') : 500;
			$parts = ceil($total / $limit) - 1;

			$show_price = $this->config->get('turbo_products_rss_show_price');
			$include_tax = $this->config->get('turbo_products_rss_include_tax');
			$show_image = $this->config->get('turbo_products_rss_show_image');


			if ($show_image) {
				$image_width = $this->config->get('turbo_products_rss_image_width') ? $this->config->get('turbo_products_rss_image_width') : 100;
				$image_height = $this->config->get('turbo_products_rss_image_height') ? $this->config->get('turbo_products_rss_image_height') : 100;
			}
			...
			$output .= '<link>' . $this->prepareLink($server) . '</link>';
			$output .= '<description>' . $this->prepareField($this->config->get('config_meta_description')) . '</description>';
			if (is_file(DIR_IMAGE . $this->config->get('config_logo'))) {
				$output .= '<turbo:logo>'. $this->prepareLink($server . 'image/' . $this->config->get('config_logo')) .'</turbo:logo>';
			}
        	$output .= '<turbo:cms_plugin>9C88AA168216899AC761A31118E008F1</turbo:cms_plugin>';
			foreach ($products as $product) {
					...
					if ($product['description']) {
						$description .= $this->prepareDesc($product['description']);
					}
	...
	private function prepareField($field) {
		if (!is_array($field)){
			...
			$from = array('"', '>', '<', '’', '«', '»', '“', '”', '–', '—', 'º', '®', '©', '|', '™', '\'', '&rsquo;', '&reg;', '&mdash;', '&ndash;', '&#39;', '&quot;', '&amp;', '&trade;', '&nbsp;', '&');
			$to = array('', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', ' ', '');
