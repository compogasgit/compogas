	...
	public function getCategoryTags($category_info) {
		...
		if ('nofollow' == $generate_mode_category_h1) {
			...
			if ($h1) {
				...
				$category_info['name'] = $this->stg->parse($category_info['name'], $var_values); // A! OpenCart Initial - for catalog ONLY
			}
		}
		...
		if ('empty' == $generate_mode_category_h1) {
			if ($h1) {
				...
				if (empty($category_info['name'])) {
					$category_info['name'] = $this->stg->parse($f_h1, $var_values);
				} else {
					$category_info['name'] = $this->stg->parse($category_info['name'], $var_values); // A! OpenCart Initial - for catalog ONLY
				}
			}
		...
		if ('forced' == $generate_mode_category_h1) {
			if ($h1) {
				...
				$category_info['name'] = $this->stg->parse($f_h1, $var_values); // A! OpenCart Initial - for catalog ONLY
			}
		}
	...
	public function getProductTags($data) {
		...
		if ('nofollow' == $generate_mode_product_h1) {
			...
			if ($h1) {
				...
				$product_info['name'] = $this->stg->parse($product_info['name'], $var_values); // A! OpenCart Initial - for catalog ONLY
			}
		}
		...
		if ('empty' == $generate_mode_product_h1) {
			if ($h1) {
				...
				if (empty($product_info['name'])) {
					$product_info['name'] = $this->stg->parse($f_h1, $var_values);
				} else {
					$product_info['name'] = $this->stg->parse($product_info['name'], $var_values); // A! OpenCart Initial - for catalog ONLY
				}
			}
		...
		if ('forced' == $generate_mode_product_h1) {
			if ($h1) {
				...
				$product_info['name'] = $this->stg->parse($f_h1, $var_values); // A! OpenCart Initial - for catalog ONLY
			}
		}
