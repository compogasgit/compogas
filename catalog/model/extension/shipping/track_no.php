...
class ModelExtensionShippingTrackNo extends Model {
	...
	public function save($order_id, $track_no) {
		if (!$this->config->get('shipping_track_no_status') && !$this->request->post['track_no_status']) {
			return false;
		}
	...
	**/
	public function exportLiveinform($api_id, $order_info) {
		...
		$body = file_get_contents('http://www.liveinform.ru/api/add/?api_id='.$api_id.'&phone='.$phone.'&tracking='.urlencode($order_info['track_no'])
			.'&type=2&order_id='.urlencode($order_info['order_id']).'&email='.urlencode($order_info['email'])
