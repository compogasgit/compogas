...
class ControllerExtensionModuleMicrodataPro extends Controller {
		...
		if($this->config->get('module_microdatapro_status') && $this->config->get('module_microdatapro_company') && $this->microdatapro->key($this->config->get('module_microdatapro_license_key'))){
			...
			foreach ($query_stores->rows as $result){
				if($result['url'] == $this->host){
					...
					$data['organization_phones']= $this->config->get('module_microdatapro_phones'.$store_id)?array_diff(array_map('trim', explode(",", $this->config->get('module_microdatapro_phones'.$store_id))), array('')):$data['organization_phones'];
					$data['organization_groups']= $this->config->get('module_microdatapro_groups'.$store_id)?array_diff(array_map('trim', explode(",", $this->config->get('module_microdatapro_groups'.$store_id))), array('')):$data['organization_groups'];
					$data['organization_map']	= $this->config->get('module_microdatapro_map'.$store_id)?$this->config->get('module_microdatapro_map'.$store_id):$data['organization_map'];
	...
	public function category($microdatapro_data) { //Breadcrumbs - on category pages
		if($this->config->get('module_microdatapro_status') && $this->config->get('module_microdatapro_category') && $this->microdatapro->key($this->config->get('module_microdatapro_license_key'))){
			...
			if($data['range']){
				$min_max = array();
				if($microdatapro_data['results'] && count($microdatapro_data['products']) > 1){
					foreach($microdatapro_data['results'] as $product){
						$min_max[] = $product['special']?$product['special']:$product['price'];
					}
				}
				if(count($min_max) > 1){
	...
	public function product($microdatapro_data) { //Product - on product page
		if($this->config->get('module_microdatapro_status') && $this->config->get('module_microdatapro_product') && $this->microdatapro->key($this->config->get('module_microdatapro_license_key'))){
			...
			$data['popup']		 = str_replace(array('id','"',"'",'mainimage'), "", $microdatapro_data['popup']); //fix zoom module id="mainimage"
			$data['thumb']		 = $microdatapro_data['thumb'];
			...
			$data['images'] = false;
			if($this->config->get('module_microdatapro_product_gallery')){
			...
			if(!$this->config->get('module_microdatapro_hide_price')){
				...
				//option price
				$option_prices = array();
				if(!(int)$data['price'] && $microdatapro_data['options'] && !$this->config->get('module_microdatapro_hide_price')){
