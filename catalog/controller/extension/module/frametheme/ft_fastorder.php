...
class ControllerExtensionModuleFramethemeFTFastorder extends Controller {
	...
	public function index() {
		...
		if (isset($ft_settings['t1_fastorder_phone_mask']) && $ft_settings['t1_fastorder_phone_mask']) {
			...
			$data['phone_mask'] = $this->config->get('config_email');
		}
		...
		if ($product_info) {
			...
			if ($product_info['image']) {
				...
				$data['thumb2x'] = $this->model_tool_image->resize($product_info['image'], 220*2, 220*2);
				$data['thumb3x'] = $this->model_tool_image->resize($product_info['image'], 220*3, 220*3);
				$data['thumb4x'] = $this->model_tool_image->resize($product_info['image'], 220*4, 220*4);
			} else {
				$data['thumb'] = '';
			}
